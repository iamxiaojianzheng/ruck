...
function convertEntryFile2Feature(appPath: string): AppInfo | null {
  ...
  if (targetAppInfo.NotShowIn && targetAppInfo.NotShowIn.toLowerCase().includes(os)) {
    return null;
  }
  ...
  if (icon.startsWith('/')) {
    ...
    if (!appPath.startsWith((window as any).process.env.HOME + '/.local/share/applications')) return null;
    appPath = path.join((window as any).process.env.HOME, '.local/share/icons', appPath + '.png');
...
function getIcon(filePath: string) {
  ...
  const types = ['apps', 'categories', 'devices', 'mimetypes', 'legacy', 'actions', 'places', 'status', 'mimes'];
  const exts = ['.png', '.svg'];
  for (const theme of themes) {
    for (const size of sizes) {
      for (const type of types) {
        for (const ext of exts) {
          let iconPath = path.join('/usr/share/icons', theme, size, type, filePath + ext);
          if (originfs.existsSync(iconPath)) return iconPath;
          iconPath = path.join('/usr/share/icons', theme, type, size, filePath + ext);
          if (originfs.existsSync(iconPath)) return iconPath;
        }
      }
    }
