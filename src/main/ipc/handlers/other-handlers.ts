...
export const addLocalStartPlugin: IPCHandler<'localStart:add'> = (event, { plugin }) => {
  ...
  window.webContents.executeJavaScript(`window.addLocalStartPlugin(${JSON.stringify({ plugin })})`);
};
...
export const removeLocalStartPlugin: IPCHandler<'localStart:remove'> = (event, { plugin }) => {
  ...
  window.webContents.executeJavaScript(`window.removeLocalStartPlugin(${JSON.stringify({ plugin })})`);
};
